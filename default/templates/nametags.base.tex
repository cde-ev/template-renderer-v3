                    ...
                    \begin{lrbox}{\ntcontent}
                        \fbox{\hspace{3mm}\begin{minipage}[b][5.5cm][t]{8.4cm}
                            ...
                            <<%- block nametag_frontside scoped %>>
                                ...
                                \\[1mm]%
                                % Course(s)
                                <<% with courses, merged = get_courses(p, part.tracks[:2] if part else EVENT.tracks[:2]) -%>>
                                    <<% for course in courses -%>>
                                        ...
                                        \parbox[t][17mm][c]{<<% if courses|length > 1 %>>25.4mm<<% else %>>58.9mm<<% endif %>>}{
                                            \begin{flushleft}
                                                {<<<course.shortname|e>>><<% if merged %>><<<CONFIG.nametags.merge_courses_suffix>>><<% endif %>>}
                                            \end{flushleft}
                                        }%
                    ...
                    \hspace{5mm}\begin{minipage}[t][5.5cm][t]{8.0cm}%
                        ...
                        <<%- block nametag_rearside scoped %>>
                            <<%- block nametag_reartext scoped %>>
                                <<% with courses, merged = get_courses(p, part.tracks[:2] if part else EVENT.tracks[:2], True) -%>>
                                    <<% if CONFIG.data.course_room_field and courses %>>
