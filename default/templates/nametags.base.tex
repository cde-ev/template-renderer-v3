...
<<% block content %>>
    ...
    \begin{itemize}
        ...
        <<% for block_name, participants in participant_blocks %>>
            <<% set num_sheets = (participants|length/8)|round(method='ceil') %>>
    ...
    Stand: <<< EVENT.timestamp.strftime('%d.%m.%Y %H:%M') >>>
    ...
    <<% for block_name, participants in participant_blocks %>>
        ...
        <<% for list in participants|batch(8) -%>>
                            ...
                            <<%- block nametag_frontside scoped %>>
                                ...
                                \parbox[t][1.1cm][c]{5cm}{
                                    <<% block nametag_lodgement -%>>
                                        <<% for part in EVENT.parts[:2] -%>>
                                            {<<% if part in p.parts and p.parts[part].lodgement %>><<< p.parts[part].lodgement.moniker|e >>><<% endif %>>}
                                ...
                                \\[1mm]%
                                % Course(s)
                                <<% for track in EVENT.tracks[:2] -%>>
                                    <<% if track in p.tracks and p.tracks[track].course -%>>
                                        <<% with course = p.tracks[track].course %>>
                                            % Course icon
                                            ...
                                            <<% endif -%>>
                                            % Course name
                                            \parbox[t][17mm][c]{25.4mm}{   <<# TODO set to 58.9mm if only one course is shown #>>
                                                \begin{flushleft}
                                                    {<<<course.shortname|e>>>}
                                                \end{flushleft}
                                            }%
                    ...
                    \hspace{5mm}\begin{minipage}[t][5.5cm][t]{8.0cm}%
                        ...
                        <<%- block nametag_rearside scoped %>>
                            <<%- block nametag_reartext scoped %>>
                                <<# TODO show only, if participant has at least one track with courses
                                <<% if CONFIG.data.course_room_field and EVENT.tracks %>>
                                    Kursraum:
                                    <<% for track in EVENT.tracks[:2] if (track in p.tracks and p.tracks[track].course) -%>>
                                        <<< p.tracks[track].course.fields[CONFIG.data.course_room_field]|e ->>>
                                        <<% if not loop.last %>>, <<% endif %>>
                                    <<%- endfor -%>>
                                <<% endif %>>
                            <<% endblock %>>
