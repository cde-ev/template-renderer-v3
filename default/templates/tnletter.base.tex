...
\usepackage[
    ...
    linkcolor=links,
    urlcolor=links,
    pdftitle={Teilnehmerbrief <<< EVENT.title|e >>> – <<< participant.name.fullname|e >>>},
...
\begin{document}
    ...
    \begin{letter}{
        <<< participant.name.fullname|e >>>\\
        <<< participant.address.full_address|e(linebreaks=True) >>>
    }

        \setkomavar{subject}{<<% block subject %>>Teilnehmerbrief <<< EVENT.title|e >>><<% endblock %>>}

        \opening{<<% block opening %>>Hallo <<< participant.name.given_names|e >>>,<<% endblock %>>}
